...
const card = `import { ReactNode } from 'react';
...
const index = `export { Button } from './Button';
export { Input } from './Input';
export { Modal } from './Modal';
export { Card } from './Card';
`;

fs.writeFileSync(path + 'Button.tsx', button);
fs.writeFileSync(path + 'Input.tsx', input);
fs.writeFileSync(path + 'Modal.tsx', modal);
fs.writeFileSync(path + 'Card.tsx', card);
fs.writeFileSync(path + 'index.ts', index);

console.log('✅ All UI components written');
